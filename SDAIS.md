...
| Initialiser, Updater | Any capable tier | Template extraction and scaffolding are low-complexity tasks |
...
├── AGENTS.md
└── sdais/
    ├── SDAIS.md              ← paradigm specification   ┐
    ├── SDAIS-INIT.md         ← bootstrap init prompt    │ distribution set
    ├── SDAIS-UPDATE.md       ← bootstrap update prompt  ┘
    ├── prompts/              ← generated by Initialiser; refreshed by Updater
    │   ├── init.md           ← copy of SDAIS-INIT.md
    │   ├── update-sdais.md   ← copy of SDAIS-UPDATE.md
    │   ├── semantic-auditor.md
    │   ├── grounder.md
    │   ├── designer.md
    │   ├── generator.md
...
- The three distribution files (`SDAIS.md`, `SDAIS-INIT.md`, `SDAIS-UPDATE.md`) are the only files a human copies when starting or upgrading a project. All other files under `sdais/` are generated or authored in place.
- The `prompts/` directory is created by the Initialiser and refreshed by the Updater. `prompts/init.md` is a verbatim copy of `SDAIS-INIT.md`; `prompts/update-sdais.md` is a verbatim copy of `SDAIS-UPDATE.md`. The remaining prompt files are extracted from `SDAIS.md`.
...
Before authoring any RSF items, run the Initialiser agent once to scaffold the `sdais/` directory tree. Provide the project name. Use `sdais/SDAIS-INIT.md` directly as the prompt — it is one of the three distribution files and exists before `sdais/prompts/` is created.

After init completes, `sdais/prompts/init.md` is a verbatim copy of `SDAIS-INIT.md` and serves as the prompt reference for subsequent invocations.

**Prompt (`sdais/SDAIS-INIT.md` / `sdais/prompts/init.md`):**

The full prompt is in `sdais/SDAIS-INIT.md` / `sdais/prompts/init.md`. See those files for the complete text.

### Step −1b — Update Scaffolding to a New SDAIS Version

When SDAIS.md itself is updated (new paradigm version), the human replaces the three distribution files (`SDAIS.md`, `SDAIS-INIT.md`, `SDAIS-UPDATE.md`) in `sdais/`, then runs the Updater agent using `sdais/SDAIS-UPDATE.md` as the prompt. The Updater refreshes all scaffolding without disturbing project-specific content.

**What is scaffolding vs. project content:**

| Refreshed by Updater | Left untouched |
|---|---|
| `AGENTS.md` | `sdais/rsf/v*/fr-NNNN-*.md` (NNNN ≥ 0001) |
...
The full prompt is in `sdais/SDAIS-UPDATE.md` / `sdais/prompts/update-sdais.md`. See those files for the complete text.
