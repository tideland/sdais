...
- **TransformationEngineer agent** (`sdais/prompts/transformation-engineer.md`) —
  first-class peer of RequirementsEngineer for the transformation workflow. Operates
  in the same two auto-detected modes with the same `[[QN]]/[[AN]]` clarification
  protocol, but reads from `sdais/tspec/v<N>/` and produces:
  - TRS hypothesis items in `sdais/trs/v1/`, each with a `**Confidence:**` field
    (High / Medium / Low) inferred from answer evidence quality, and a `**Source:**`
    field.
  - CDF files in `sdais/cdf/v1/` with `**Status:** Draft`; human promotes to
    `Active` before running the Transformation agent.
- **SpecificationEngineer archetype** — RequirementsEngineer and
  TransformationEngineer are named as the two built-in instances of the
  SpecificationEngineer pattern (prose in → formal spec out → `[[QN]]/[[AN]]`
  refinement loop). Documented in `SDAIS.md` and `scaffold/AGENTS.md`.
- **`sdais/spec/` directory** — versioned directory tree for pre-RSF prose.
- **`sdais/tspec/` directory** — versioned directory tree for transformation
  prose input (parallel to `spec/`).
- **`sdais/trs/` directory** (replaces `sdais/res/`) — versioned directory tree
  for TRS hypothesis items.
- **TRS item templates**: `trs-fr-0000-template.md`, `trs-nfr-0000-template.md`,
  `trs-c-0000-template.md` added to `scaffold/sdais/trs/v1/`.
- **`**Source:**` field** on all five RSF item templates and on CDF files. Also
  present on TRS item templates.
- **`**Status:** Draft`** value for CDF files — generated by TransformationEngineer;
  must be promoted to `Active` by the human before the Transformation agent runs.
- **Step −2 — Requirements Engineering** in `SDAIS.md` — normative section
  (five sub-steps) documenting the prose-to-RSF workflow.
- **SDAIS-T Step −2 — Transformation Engineering** in `SDAIS.md` — symmetric
  normative section (five sub-steps) documenting the prose-to-TRS+CDF workflow.
- **Step 0** added to `docs/GREENFIELD.md` — optional RequirementsEngineer
  on-ramp with updated Mermaid diagram.
- **Step 0** added to `docs/TRANSFORMATION.md` — optional TransformationEngineer
  on-ramp with updated Mermaid diagram.

### Changed
- **Full rename: "re-engineering" → "transformation"** across all documentation,
  artefact names, directory names, and agent names:
  - SDAIS-RE → SDAIS-T
  - `docs/RE-ENGINEERING.md` → `docs/TRANSFORMATION.md`
  - Re-engineering agent → Transformation agent
  - `scaffold/sdais/prompts/re-engineering.md` → `scaffold/sdais/prompts/transformation.md`
  - RES (Re-engineering Specification) → TRS (Transformation Specification)
  - `sdais/res/v<N>/` → `sdais/trs/v<N>/`
  - `res-fr-`, `res-nfr-`, `res-c-` item prefixes → `trs-fr-`, `trs-nfr-`, `trs-c-`
  - `(AGENT) Re-engineering` → `(AGENT) Transformation`
  - `RES-CONTRADICTS-CODE` RAR category → `TRS-CONTRADICTS-CODE`
- **Transformation prompt** (`transformation.md`): `(AGENT)` value updated to
  `Transformation`; references to `res/` updated to `trs/`; TRS item ID format
  updated to `TRS-FR-NNNN` etc.
- **Analyzer prompt**: `(ORIGIN)` now set to confirmed TRS item IDs where a
  confident mapping exists; finding category updated to `TRS-CONTRADICTS-CODE`.
- **Model tier table** in `SDAIS.md`: TransformationEngineer row added (High
  reasoning recommended); Transformation agent row updated.
- **Agent Role Values table** in `SDAIS.md`: `TransformationEngineer` and
  `Transformation` entries; `Re-engineering` removed.
- **Annotation mutation rules table** in `SDAIS.md`: `Re-engineering` row renamed
  to `Transformation`.
- **`(CONFIDENCE)` label** in `SDAIS.md`: now written by TransformationEngineer
  (on TRS items) in addition to Analyzer (on ANN blocks).
- **Directory structure** in `SDAIS.md` and `AGENTS.md`: `res/` → `trs/`; `tspec/`
  added; `transformation-engineer.md` and `transformation.md` in `prompts/`.
- **Version directory semantics** in `SDAIS.md`: `tspec/` and `trs/` semantics
  documented.
- **Update table** in `SDAIS.md`: `tspec/v*/*.md` and `trs/v*/*.md` added to the
  "left untouched" column.
- **`docs/INTRODUCTION.md`**: TransformationEngineer added to the agent table;
  "SDAIS-RE" → "SDAIS-T"; link updated from `RE-ENGINEERING.md` to `TRANSFORMATION.md`.
- **`docs/GLOSSARY.md`**: SDAIS-RE → SDAIS-T; RES section renamed to TRS; prefixes
  updated; Agent Role Values updated.
- **`README.md`**: "The Human Loop — Re-Engineering" → "The Human Loop —
  Transformation"; Phase 0 (TransformationEngineer) added; link updated.
